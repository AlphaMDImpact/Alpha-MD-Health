...
{
    /// <summary>
    /// represents Entry Validation Behavior
    /// </summary>
    public class EntryValidationBehavior : Behavior<CustomEntry>
    {
        ...
        private void SetEntryValidation(EventArgs args, CustomEntry entry)
        {
        ...
        {
            IsValid = ValidateControl((CustomEntry)sender);
            if (IsValid)
            {
        ...
        private bool ValidateControl(CustomEntry entry)
        {
            bool isValid = false;
            decimal tryOutput = 0;
        ...
        private bool DisplayValidationRangeErrors(CustomEntry entry, bool isValid, int errorIndex)
        {
        ...
        private bool DisplayErrors(CustomEntry entry, bool isValid, int errorIndex)
        {
        ...
        private void ValidateControls(CustomEntry entry, ref bool isValid, ref decimal tryOutput, string entryText, int errorIndex)
        {
        ...
        private bool ValidateRange(CustomEntry entry)
        {
            bool isValid = false;
            if (!string.IsNullOrWhiteSpace(entry.Text))
            {
        ...
        private bool ValidatePhoneNumber(CustomEntry entry, string entryText)
        {
        ...
        private bool ValidationRegxString(CustomEntry entry, string entryText)
        {
            bool isValid;
            try
            {
        ...
        private bool CheckMinLengthValidation(CustomEntry entry, string entryText)
        {
        ...
        private bool ValidateMinimumLength(CustomEntry entry, string entryText)
        {
            bool isValid = false;
            if (!string.IsNullOrWhiteSpace(entryText))
            {
        ...
        private bool CheckValidationRange(CustomEntry entry)
        {
            bool isValid = false;
            CultureInfo metricCulture = CultureInfo.GetCultureInfo(Constants.ENGLISH_US_LOCALE);
            _rangeValidationText = entry.Range?.Split(Constants.SYMBOL_COLAN);
