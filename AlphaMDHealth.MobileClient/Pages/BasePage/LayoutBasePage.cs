...
public partial class BasePage : ContentPage, IDisposable
{
    ...
    /// </summary>
    internal AmhMessageControl MessagePopup { get; set; }

    ///// <summary>
    ///// View used to display sync status
    ///// </summary>
    //public SyncStatusView SyncStatusView { get; private set; }//todo:
    ...
    public BasePage(PageLayoutType pageLayoutType, bool withScrollBar)
    {
        ...
        MainThread.BeginInvokeOnMainThread(() =>
        {
            ...
            {
                StatusBarColor = (Color)App.Current.Resources[StyleConstants.ST_PRIMARY_APP_COLOR],
        ...
        _statusMessage = new AmhLabelControl(FieldTypes.LightSmallHVCenterBoldLabelControl)
        {
            ResourceKey = ErrorCode.OK.ToString(),
            IsVisible = false
        };
        //todo:
        MessagePopup = new AmhMessageControl()
        {
            IsVisible = false
        };
    ...
    protected static View CreateAppLogo(bool islight, FieldTypes fieldTypes)
    {
        return new AmhImageControl(fieldTypes)
        {
            Icon = islight ? ImageConstants.I_LOGO_LIGHT_PNG : ImageConstants.I_LOGO_DARK_PNG,
    ...
    public void SetPageContent(bool withScrollBar)
    {
        ...
        MasterGrid.Add(_statusMessage, 0, 0);
    ...
    private void CreateHeaderLayout()
    {
        ...
        {
            Heading = new AmhLabelControl(FieldTypes.LightLargeHVCenterBoldLabelControl);
            Description = new AmhLabelControl(FieldTypes.LightMediumHVCenterLabelControl);
            ...
            headerContent.Add(CreateAppLogo(true, FieldTypes.SquareImageControl), 0, 0);
    ...
    /// </summary>
    /// <param name="withScrollBar">Flag which decides Base page layout grid will contain scrollbar or not</param>
    public void ApplyPageResources()
    {
        _statusMessage.PageResources = MessagePopup.PageResources = PageData;
        if (Heading != null)
        {
            Heading.PageResources = PageData;
        }
        if (Description != null)
        {
            Description.PageResources = PageData;
        }
    }
    ...
    public Thickness GetLayoutPadding(PageLayoutType pageLayoutType)
    {
        ...
        {
            PageLayoutType.LoginFlowPageLayout => (double)new OnIdiom<double> { Phone = _appPadding, Tablet = p },
            PageLayoutType.MastersContentPageLayout => (double)new OnIdiom<double> { Phone = 0, Tablet = p },
            _ => (double)new OnIdiom<double> { Phone = 0, Tablet = 0 },
        };
        return new Thickness(padding, 0);
    }
    ...
    private async Task ResetLayoutAsync()
    {
        if (MobileConstants.IsTablet)
        {
            ...
            //{
            //todo:MessagePopup.ResetLayoutForMessagePopUp(_isLandscape);
            //}
    ...
    {
        PageLayout.Margin = GetLayoutPadding(value);
    }
