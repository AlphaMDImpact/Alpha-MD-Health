...
public class ShellMasterPage : Shell
{
    ...
    /// </summary>
    public BasePage BaseContentPageInstance { get; private set; }
    ...
    public BasePage CurrentPage
    {
        get
        {
            if (Current != null)
            {
                Page currentPage = (Current.CurrentItem?.CurrentItem as IShellSectionController)?.PresentedPage
                    ?? Current.Navigation?.NavigationStack?.LastOrDefault();
    ...
    public ShellMasterPage() : this(new BasePage()) { }
    ...
    {
        BaseContentPageInstance = baseContentPage;
    ...
    public async Task RenderPageAsync()
    {
        if (!_isRendering)
        {
            ...
            List<MenuModel> footerMenus = await GetTabbedMenusAsync().ConfigureAwait(true);
            if (GenericMethods.IsListNotEmpty(footerMenus))
            {
                await SetFooterMenusAsync(footerMenus).ConfigureAwait(true);
    ...
    private static void PopToRoot(ShellNavigatingEventArgs args)
    {
        if (ShouldPopToRoot(args))
        {
            _ = Current.CurrentItem.Navigation.PopToRootAsync(false);
        }
    }
    ...
    {
        Dictionary<string, string> badgeCounts = await GetBadgeCountsAsync(footerMenus).ConfigureAwait(true);
        var existingFooterItems = FooterTabs.Items.ToList();
        ...
        {
            FooterTabs.Items.Add(new CustomTab
            {
                ...
                Items =
                {
                    new ShellContent
                    {
                        ...
                        Route = $"{footerMenu.TargetPage}" + $"{(footerMenu.TargetPage.Contains(Constants.SYMBOL_QUESTION_MARK)
                        ? Constants.SYMBOL_AMPERSAND
                        : Constants.SYMBOL_QUESTION_MARK)}" +
                        ...
                        ? $"{Constants.SYMBOL_AMPERSAND}{Param.id}{Constants.SYMBOL_EQUAL}{footerMenu.Content}" +
                            $"{Constants.SYMBOL_AMPERSAND}{Param.type}{Constants.SYMBOL_EQUAL}{PageType.ContentPage}"
                        : string.Empty),
    ...
    private ImageSource GetFooterIcon(MenuModel footerMenu, bool selectedIcon)
    {
        ...
        string iconName = selectedIcon && !string.IsNullOrWhiteSpace(footerMenu.SelectedGroupIcon)
            ? footerMenu.SelectedGroupIcon
            : footerMenu.GroupIcon;
        //todo: need to confirm below code changes
    ...
    /// Get footer menus
    /// </summary>
    /// <returns>List of footer menus</returns>
    protected async Task<List<MenuModel>> GetTabbedMenusAsync()
    {
        ...
        if (menuData.ErrCode == ErrorCode.OK)
        {
