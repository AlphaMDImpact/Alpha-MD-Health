...
namespace AlphaMDHealth.IntegrationServiceLayer
{
    ...
    public class ExtendedHttpActionResult<T> : IActionResult where T : BaseDTO
    {
        ...
        {
            ResponseObject.ErrorDescription = StatusDescription;
                //await new BaseServiceBusinessLayer(context.HttpContext)
                //.GetResourceValueByKeyAsync(GroupConstants.RS_COMMON_GROUP, ResponseObject.ErrCode.ToString(), LanguageID).ConfigureAwait(false);
